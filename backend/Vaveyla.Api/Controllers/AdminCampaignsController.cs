...
public sealed class AdminCampaignsController : ControllerBase
{
    ...
    public async Task<ActionResult<List<CampaignDto>>> GetAll(CancellationToken ct)
    {
        ...
        var restaurantNames = new Dictionary<Guid, string>();
        foreach (var rid in restaurantIds)
        {
            var r = await _restaurantRepo.GetRestaurantByIdAsync(rid, ct);
            if (r != null) restaurantNames[rid] = r.Name;
        }

        var result = campaigns.Select(c => ToDto(c, restaurantNames.GetValueOrDefault(c.RestaurantId ?? Guid.Empty))).ToList();
        return Ok(result);
    }
