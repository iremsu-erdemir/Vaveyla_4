...
class _ChangeAddressScreenState extends State<ChangeAddressScreen> {
  ...
  String _toUserFriendlyError(Object error) {
    ...
    if (raw.contains('contains invalid characters') ||
      ...
      return 'Gecersiz giris. Lutfen sadece uygun karakterler kullanin.';
    }
    if (raw.contains('formatexception')) {
      return 'Gecersiz giris formati.';
    }
  ...
  Future<void> _addNewAddress(String address) async {
    ...
    if (created != null && mounted) {
      setState(() {
        _markSelectedLocally(created.addressId);
        _savedAddresses.insert(0, created.copyWith(isSelected: true));
      });
    }
  ...
  Future<void> _startEditAddressFlow(UserAddress address) async {
    ...
    final normalizedAddress =
        selectedAddress != null && selectedAddress.trim().isNotEmpty
  ...
  Future<void> _startAddAddressFlow() async {
    ...
    if (selectedAddress == null || selectedAddress.trim().isEmpty || !mounted) {
      return;
    }

    await _addNewAddress(selectedAddress.trim());
  }
